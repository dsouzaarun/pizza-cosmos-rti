...
$tiles = @(
    (New-Tile -Title "Active Orders" -Query "Orders | where Timestamp > ago(1h) | summarize arg_max(Timestamp, *) by OrderId | where Status !in ('delivered', 'cancelled') | count" -VisualType "multistat" -X 0 -Y 0 -W 12 -H 6 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Drivers Online" -Query "DriverUpdates | where Timestamp > ago(5m) | summarize arg_max(Timestamp, *) by DriverId | where Status != 'offline' | count" -VisualType "multistat" -X 12 -Y 0 -W 12 -H 6 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Avg Delivery (min)" -Query "Orders | where Timestamp > ago(1h) | where Status == 'delivered' | summarize AvgMinutes = round(avg(EstimatedDeliveryMinutes), 0)" -VisualType "multistat" -X 0 -Y 6 -W 12 -H 6 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "SLA %" -Query "SlaCompliance(1h) | project SlaPercent" -VisualType "multistat" -X 12 -Y 6 -W 12 -H 6 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Active Alerts" -Query "Alerts | where Timestamp > ago(1h) | where IsResolved == false | count" -VisualType "multistat" -X 0 -Y 12 -W 12 -H 6 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Orders Per Minute" -Query "OrdersPerMinute(30m)" -VisualType "line" -X 12 -Y 12 -W 12 -H 8 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Kitchen Utilization" -Query "KitchenLoadHeatmap() | project KitchenName, UtilizationPercent" -VisualType "bar" -X 0 -Y 20 -W 12 -H 8 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Driver Fleet Status" -Query "DriverFleetStatus() | project DriverName, Status, CurrentOrderId, Speed" -VisualType "table" -X 12 -Y 20 -W 12 -H 8 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "Alert Feed" -Query "Alerts | where Timestamp > ago(1h) | where IsResolved == false | project Timestamp, Severity, RuleName, Message, RecommendedAction | order by case(Severity == 'critical', 0, Severity == 'high', 1, Severity == 'medium', 2, 3) asc | take 20" -VisualType "table" -X 0 -Y 28 -W 12 -H 8 -PageId $pageId -DsId $dsId),
    (New-Tile -Title "VIP Order Tracker" -Query "Orders | where Timestamp > ago(2h) | where IsVip == true | summarize arg_max(Timestamp, *) by OrderId | where Status !in ('delivered', 'cancelled') | project OrderId, CustomerName, Status, KitchenName, EstimatedDeliveryMinutes | order by EstimatedDeliveryMinutes desc" -VisualType "table" -X 12 -Y 28 -W 12 -H 8 -PageId $pageId -DsId $dsId)
)
