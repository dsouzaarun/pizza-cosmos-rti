...
# Row 1: KPI strip (2 multistat cards)
...
# Row 2: More KPIs (2 per row, min width 12)
$t1_avgDelivery = New-Tile -Title "Avg Delivery (min)" `
    ...
    -VisualType "multistat" -X 0 -Y 6 -W 12 -H 6 -PageId $page1Id -DsId $dsId
...
$t1_sla = New-Tile -Title "SLA Compliance %" `
    ...
    -VisualType "multistat" -X 12 -Y 6 -W 12 -H 6 -PageId $page1Id -DsId $dsId

# Row 3: Live map + Active Alerts KPI
$t1_alerts = New-Tile -Title "Active Alerts" `
    -Query "Alerts | where Timestamp > ago(1h) | where IsResolved == false | count" `
    -VisualType "multistat" -X 0 -Y 12 -W 12 -H 6 -PageId $page1Id -DsId $dsId
...
$t1_ordersTrend = New-Tile -Title "Orders Per Minute (30m)" `
    ...
    -VisualType "timechart" -X 14 -Y 18 -W 10 -H 12 -PageId $page1Id -DsId $dsId

# Row 5: Order Pipeline + Alert Feed
$t1_pipeline = New-Tile -Title "Order Pipeline" `
    ...
    -VisualType "table" -X 0 -Y 30 -W 10 -H 8 -PageId $page1Id -DsId $dsId
...
$t1_alertFeed = New-Tile -Title "Alert Feed" `
    ...
    -VisualType "table" -X 10 -Y 30 -W 14 -H 8 -PageId $page1Id -DsId $dsId
...
$t2_utilBar = New-Tile -Title "Kitchen Utilization %" `
    -Query "KitchenMetrics | where Timestamp > ago(5m) | summarize arg_max(Timestamp, *) by KitchenId | extend UtilizationPct = round(UtilizationPercent, 1) | project KitchenName, UtilizationPct | order by UtilizationPct desc" `
...
$t2_heatmap = New-Tile -Title "Kitchen Load Heatmap" `
    -Query "KitchenMetrics | where Timestamp > ago(30m) | summarize AvgQueue = avg(QueueDepth) by KitchenName, TimeBin = bin(Timestamp, 5m) | project KitchenName, TimeBin, AvgQueue" `
    -VisualType "heatmap" -X 0 -Y 20 -W 12 -H 10 -PageId $page2Id -DsId $dsId

$t2_kitchenTable = New-Tile -Title "Kitchen Details (Live)" `
    -Query "KitchenMetrics | where Timestamp > ago(5m) | summarize arg_max(Timestamp, *) by KitchenId | project KitchenName, Status, QueueDepth, Capacity, AvgPrepTime = round(AvgPrepTimeMinutes, 1), Utilization = strcat(tostring(round(UtilizationPercent, 0)), '%') | order by QueueDepth desc" `
...
$t3_fleetTable = New-Tile -Title "Fleet Status (Live)" `
    -Query "DriverUpdates | where Timestamp > ago(5m) | summarize arg_max(Timestamp, *) by DriverId | project DriverId, Status, Speed = round(Speed, 1), CurrentOrderId, Lat = round(Latitude, 4), Lng = round(Longitude, 4) | order by Status asc, DriverId asc" `
