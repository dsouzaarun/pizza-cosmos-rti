...
$eventstreams = az rest --method GET `
    --url "https://api.fabric.microsoft.com/v1/workspaces/$WorkspaceId/eventstreams" `
...
if ($existing) {
    Write-Host "  ℹ️  EventStream '$EventStreamName' already exists (ID: $($existing.id))" -ForegroundColor Cyan
    ...
    $body = @{
        ...
        description = "Real-time pizza delivery event stream for Pizza Cosmos"
